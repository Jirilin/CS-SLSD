...
from metrics import (
    accuracy, class_accuracy, snapshot, parameter_change, forgetting_from_history,
    task_accuracy_vector, average_incremental_accuracy, backward_transfer_proxy,
...
def parse_args():
    ...
    p.add_argument("--smoke", action="store_true", help="Fast two-batch pipeline validation")
    return p.parse_args()


def main():
    args = parse_args()
    overrides = {"seed": args.seed, "ewc_lambda": args.ewc_lambda,
                 "confidence_threshold": args.threshold}
    if args.initial_epochs is not None: overrides["initial_epochs"] = args.initial_epochs
    if args.stream_batches is not None: overrides["stream_batches"] = args.stream_batches
    if args.fisher_samples is not None: overrides["fisher_samples"] = args.fisher_samples
    if args.smoke:
        overrides.update(initial_epochs=1, stream_batches=2, fisher_samples=100)
    ...
    task_df = pd.DataFrame(task_hist).rename(columns=lambda c: f"task_{c}_accuracy")
    task_df.insert(0, "batch", [-1] + list(range(cfg.stream_batches)))
    task_df.to_csv(out / f"task_accuracy_{stem}.csv", index=False)
    summary = {
        ...
        "stability_plasticity_diagnostic": stability_plasticity_score(
            float(df.iloc[-1].test_accuracy), forgetting_from_history(class_hist),
            float(df.parameter_change.mean())
        ),
    }
