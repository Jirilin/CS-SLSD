...
def confidence_pseudo_labels(model, images, hidden_labels, device, threshold):
    ...
    with torch.no_grad():
        ...
        # Evaluation only: hidden labels are not used to select or train
        # pseudo-labels. Move all evaluation tensors to CPU before masking.
        pred_cpu = pred.detach().cpu()
        mask_cpu = mask.detach().cpu()
        hidden_labels_cpu = hidden_labels.detach().cpu()

        if mask_cpu.any().item():
            precision = float(
                (
                    pred_cpu[mask_cpu]
                    == hidden_labels_cpu[mask_cpu]
                )
                .float()
                .mean()
                .item()
