    ...
    for batch in stream.batches():
        if args.method == "naive":
            ...
            with torch.no_grad():
                ...
                mask_cpu = mask.cpu()
                precision = float((pred[mask].cpu() == batch.hidden_labels[mask_cpu]).float().mean()) if mask.any() else np.nan
                agreement = np.nan
        else:
        ...
        # Handle replay buffer addition & sampling
        ...
        # Snapshot parameters and track evaluation metrics
